...
import { DEMO_MODE_KEY, DEMO_TASKS_KEY, DEVICE_ID_KEY, NOTIF_MAP_KEY, REAL_KEY } from './keys';
...
export function TasksProvider({ children }: { children: React.ReactNode }) {
  ...
  const scheduleNotifFor = useCallback(
    async (task: Task) => {
      // Any change resets the schedule (and clears an existing snooze).
      ...
      const id = await scheduleReminder(when, task.title, task.notes || undefined, task.id);
      if (id) {
        const map = await loadNotifMap();
        map[task.id] = id;
        await saveNotifMap(map);
  ...
  const addTask = useCallback(
    async (draft: Omit<Task, 'id' | 'createdAt' | 'done'> & { done?: boolean }) => {
  ...
  // Re-insert a previously-deleted task as-is (for undo). Keeps the same id and
  // re-schedules its reminder.
  const restoreTask = useCallback(
    async (task: Task) => {
      if (tasks.some((t) => t.id === task.id)) return;
      const next = [task, ...tasks];
      await persist(next);
      await scheduleNotifFor(task);
      if (!demoMode) syncPushTask(task).catch(() => {});
    },
    [tasks, persist, scheduleNotifFor, demoMode]
  );
    ...
    if (v) {
      ...
      for (const t of real) {
        ...
        await (async () => {
          ...
          const id = await scheduleReminder(when, t.title, t.notes || undefined, t.id);
          if (id) {
  ...
  const value: Ctx = useMemo(
    ...
    [ready, tasks, demoMode, snoozes, notifPermission, refreshSnoozes, addTask, updateTask, deleteTask, restoreTask, toggleDone, setDemoMode]
  );
